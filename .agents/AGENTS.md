...
- **Find who to write with `agent-find <topic>`.** It greps every session transcript, ranks matches, and prints each agent's handle, title, liveness, scratchpad, and resume command. Use it before duplicating work another session already did, before editing an artifact whose owner you do not know, and to answer "which agent worked on X?" without asking Josh. A `RUNNING` result can be mailed and will see the notice; a parked one may need its resume command run first.
