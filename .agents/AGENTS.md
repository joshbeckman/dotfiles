...
- **Waiting mail is announced.** When a message is first seen, a timestamped `agent-mail` notice appears in the conversation naming the sender and arrival time — read it as arriving *then*, not as a fact that was true at session start. Each message is announced once; `agent-mail scan --to <you>` lists whatever is still unread. There is still **no push or wakeup** — a message lands when you next take a turn, so do not assume delivery woke anyone. If you need a reply before continuing, poll (`sleep` then `scan`) rather than expecting to be interrupted.
