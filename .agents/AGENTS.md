...
- **Handles.** In prose, `@name` is a human and `@+name` is an agent (`@+simoom-farrier`). The `+` fails GitHub's and Slack's mention grammar, so an agent handle never pings a stranger, and the same `+<handle>` is the email subaddress in commit trailers. `agent-mail` accepts the sigiled forms in `--to`/`--from`.
