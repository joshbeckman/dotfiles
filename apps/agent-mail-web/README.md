...
`j` / `k` navigate conversations; `o` or Enter opens; `u` returns to the list; `/` searches; `e` archives; `c` composes; `r` replies; `a` replies to all. `g i`, `g a`, `g s`, and `g d` open Inbox, All mail, Sent, and Drafts. `?` shows help. Ctrl+Enter or Cmd+Enter sends a draft. Navigation keys do not fire while typing in inputs.
