...
- **Avatars and favicon** are local SVGs, with no Gravatar or other external lookup. Sender/contact avatars are generated deterministically from addresses, normalizing agent mailbox suffixes and address aliases. Human `@name` and agent `@+name` identities stay distinct. These are visual aids; names remain visible. The browser tab uses an envelope favicon.
