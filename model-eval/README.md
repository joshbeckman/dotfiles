...
```sh
pi-model-eval plan \
  --models openai/gpt-5.6-sol:high,anthropic/claude-sonnet-4-5:high
```
...
- `trials/*/events.jsonl` with timestamped Pi events
- `trials/*/result.json` with status, usage, timing, pricing status, and named grader checks
- `trials/*/answer.md` and a copy of the final workspace
- `report.md` with comparisons and trial failures

A timeout or observed spend stop first sends RPC `abort`, then terminates the trial process group if it does not exit. Missing or zero catalog pricing remains unknown rather than appearing as measured zero cost.

## Fixture boundary

Every trial gets a fresh temporary workspace and session. Pi starts without discovered extensions, skills, prompt templates, context files, project approval, or session persistence. The evaluation extension restricts reads and writes to relative paths inside the workspace and permits only exact shell commands named by the fixture.
