...
checks << {"name" => "rejects timeout", "required" => true, "passed" => answer.match?(/(?:not|isn.t|unlikely|unrelated).{0,40}timeout|timeout.{0,40}(?:not|isn.t|unlikely|unrelated)/) && (answer.include?("5") || answer.include?("300"))}
